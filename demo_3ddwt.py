# -*- coding: utf-8 -*-
"""
Created on Sun Aug 20 00:35:05 2017
...
data_all, label_all, X, y, height, width, num_classes, GT_Label,ind,ind_each_class = \
            load_data('indian_pines',feature_type='3ddwt',ispca=False)
...
KNN_Label = KNN.predict(data_all).reshape(width,height).astype(int).transpose(1,0)
KNN_predict_prob = KNN.predict_proba(data_all)
...
start_time = time.time()
LR = LogisticRegression(multi_class='multinomial',solver='lbfgs',C=10).fit(X_train,y_train)
LR_Label = LR.predict(data_all).reshape(width,height).astype(int).transpose(1,0)
LR_predict_prob = LR.predict_proba(data_all)
# Post-processing using Graph-Cut
